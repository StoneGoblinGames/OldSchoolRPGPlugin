...
#include "OldSchoolCharacterComponent.h"
...
UOldSchoolCharacterComponent::UOldSchoolCharacterComponent()
{
...
void UOldSchoolCharacterComponent::InitializeCharacter(FCharacterValues Values)
{
	Name = Values.Name;
	Race = Values.Race;
	Class = Values.Class;
...
int32 UOldSchoolCharacterComponent::AddExperience(int32 BaseValue, bool PrimeRequisiteBonus)
{
	int32 Bonus = 0;
...
void UOldSchoolCharacterComponent::BeginPlay()
{
	Super::BeginPlay();

	// ...
	if (DiceRollerLibrary == nullptr)
	{
		srand(time(0));
	}
...
int32 UOldSchoolCharacterComponent::GetPrimeRequisiteXPModifier()
{
	int32 Value = 0;
	...
	return UOldSchoolRPGTables::GetAttributeExperienceModifier(Value);
}

bool UOldSchoolCharacterComponent::LevelUp()
{
...
bool UOldSchoolCharacterComponent::LevelUpAdvanced(FCharacterValues LevelUpValues)
{
	return true;
}

int32 UOldSchoolCharacterComponent::GenerateRoll(FString DiceRoll)
{
	int32 Result = 0;
...
int32 UOldSchoolCharacterComponent::RollHitPoints(FString HitPointsLiteral)
{
	int32 HPModifier;
	bool AddConstitution = !HitPointsLiteral.Contains("*");
	if (AddConstitution)
	{
		HPModifier = UOldSchoolRPGTables::GetConstitutionModifier(Constitution);
	}
	else
	{
		TArray<FString> DiceRollParts;
...
int32 UOldSchoolCharacterComponent::GenerateBasicRoll(FString DiceRoll)
{
	int32 Result = 0;
	int32 Modifier = 0;
...
bool UOldSchoolCharacterComponent::AddCoins(FCoinTreasure CoinsToAdd)
{
	int32 CoinLimit = UOldSchoolRPGTables::GetCoinLimit();
...
uint8 UOldSchoolCharacterComponent::GetRetainerLimit() const
{
	return UOldSchoolRPGTables::GetMaxRetainers(Charisma);
}

bool UOldSchoolCharacterComponent::HasItemEquipped(EEquipmentSlot::Slot Slot)
{
	return EquipmentMap.Find(Slot) != nullptr;
}

void UOldSchoolCharacterComponent::EquipItem(EEquipmentSlot::Slot Slot, FEquipmentItem Item)
{
...
FEquipmentItem UOldSchoolCharacterComponent::UnEquipItem(EEquipmentSlot::Slot Slot)
{
...
// Called every frame
void UOldSchoolCharacterComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
